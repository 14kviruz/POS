...
function previsualizar() {
  ...
  if (imagen["type"] != "imagen/png" && imagen["type"] != "imagen/jpg") {
  ...
  } else if (imagen["type"] > "10000000") {
    $("#imgProducto").val("")
    swal.fire({
      icon: "error",
      showCancelButton: false,
      title: 'el archivo no puede ser mayor a 100MB'
    })
  } else {
  ...
  var obj={
    ...
    $.ajax({
        ...
        for(var i=0; i<data["listaCodigos"].length; i++){
            ...
            $("#CatProductos").append("<tr><td>"+data["listaCodigos"][i]["codigoActividad"]+"</td><td>"+data["listaCodigos"][i]
            ["codigoProducto"]+"</td><td>"+data["listaCodigos"][i]["descripcionProducto"]+"</td><td></td></tr>")
